...
(ns beatboxchad-live.fx
  (:use [overtone.core])
  )


;; TODO move the lot of these to an overtone fork
...
(defmacro def-fx
  ...
  `(defsynth ~fx-name
     ...
     (let [~'fx-arg-bus                ~'bus
           ...
           ~'[fx-arg-in-l fx-arg-in-r] ~'(* fx-arg-pre_amp (in fx-arg-bus 2))
       ...
       ~'(replace-out fx-arg-bus [fin-l fin-r]))))
...
(def-fx fx-slicer
  ...
  [phase               (varlag phase phase_slide phase_slide_curve phase_slide_shape)
   ...
   ctl-wave-prob       (buffered-coin-gate rand_buf seed probability
                                                (impulse:kr rate))
...
(without-namespace-in-synthdef
(def-fx delay-pitchshift [speed 0.1
                          decay 0.4
                          warble-multiple 4]


  [fb-loop-in (local-in:ar 1)

   processing (leak-dc:ar (* decay (delay-c:ar fb-loop-in 2 speed)))
   processing (pitch-shift:ar processing 0.2 (- 1 (* 0.03 (sin-osc:kr (* warble-multiple speed) 0)) ))
;   processing (rhpf:ar processing 200 0.8)
   fb-loop-out (local-out:ar (+ dry-l processing))
   wet-l (+ dry-l processing)
   wet-r (+ dry-r processing)]
  )
 )
